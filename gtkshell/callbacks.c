...
static void gsh_delete_GSHCBData(struct GSHCBData *cb)
{
	g_free(cb->data);
	arfree(cb);
}

static void terminal_feed(gchar * command, GtkWidget * terminal)
{
	if (!terminal) {
		ARWARN
		    ("terminal command requires a terminal to have been created");
		return;
	}
...
static bool test_special_command(struct GSHCBData *cb)
{
...
static void exec_and_test_print(struct GSHCBData *cb, const gchar * input)
{
	struct GSH *gsh;
...
static void gsh_entry_cb(GtkWidget * widget, gpointer data)
{
	struct GSHCBData *cb;
