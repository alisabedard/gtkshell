...
#ifdef HAVE_VTE
static void set_terminal_options(GSHTerminal * term)
{
	GSH *gsh;
...
static void handle_child_exited(GtkWidget * widget, gpointer term)
{
...
static gchar *get_terminal_dimension(const gint dim, const gchar * env)
{
	gchar *dstr;
	gchar *tmp;
...
static void gsh_terminal_run(GSHTerminal * term, const gchar * command)
{
...
static void gsh_delete_GSH_Terminal(GSHTerminal * term)
{
	arfree(term);
...
static void info_changed_cb(GtkWidget * widget, gpointer data)
{
	const gchar *status =
...
static void setup_terminal_signals(GSHTerminal * term)
{
	GtkWidget *widget;
	...
	if ((widget = term->widget)) {
		...
		GSHCONNECT(widget, "window-title-changed", info_changed_cb,
			   gsh);
	}
}

static GtkWidget *get_current_page(GtkNotebook * notebook)
{
...
static void
tab_button_close_cb(GtkWidget * button __attribute__ ((unused)), gpointer data)
{
...
change_page_cb(GtkNotebook * notebook,
	       gint arg1 __attribute__ ((unused)), gpointer user_data)
{
...
switch_page_cb(GtkNotebook * notebook __attribute__ ((unused)),
	       GtkWidget * page,
	       guint page_number __attribute__ ((unused)), gpointer user_data)
{
	info_changed_cb(page, user_data);
}

static void
tab_button_callback(GtkWidget * button __attribute__ ((unused)), gpointer data)
{
	GSHTerminal *term = (GSHTerminal *) data;
	$(term, add, get_shell());
}

static void tab_next(GtkWidget * button __attribute__ ((unused)), gpointer data)
{
	gtk_notebook_next_page(GTK_NOTEBOOK
			       (((GSHTerminal *) data)->__notebook));
}

static void tab_prev(GtkWidget * button __attribute__ ((unused)), gpointer data)
{
...
static void setup_action_widgets(GSHTerminal * term)
{
	GtkNotebook *notebook = GTK_NOTEBOOK(term->__notebook);
	GtkWidget *button;
	GtkWidget *row;
...
static void setup_notebook(GSHTerminal * term)
{
	GtkWidget *notebook;
...
static void new_terminal_tab(GSHTerminal * term, gchar * command)
{
	GtkNotebook *notebook = GTK_NOTEBOOK(term->__notebook);
...
static GtkWidget *gsh_GSH_Terminal_add(GSHTerminal * term,
				       const gchar * command)
{
	bool original_tab;
...
static void setup_GSH_Terminal(GSHTerminal * term)
{
	/* initialize fields */
	term->widget = NULL;
	term->font = NULL;
