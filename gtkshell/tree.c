	 ...
	 const gchar * key, const gchar * value)
{
	...
	gtk_list_store_set(store, iter, KEY_COLUMN, key, VALUE_COLUMN, value,
			   -1);
}
...
static void parse_input(const gchar * input, GtkListStore * store)
{
	GtkTreeIter iter;
	gchar **pairs;
	guint counter;
...
static GtkTreeModel *setup_model(const gchar * input)
{
	GtkListStore *store;
...
static void selection_handler(GtkTreeSelection * selection, gpointer data)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	gchar *key;
...
static void setup_signals(GSH * gsh, GtkWidget * tree_view)
{
...
GtkWidget *gsh_tree_view_new(GSH * gsh, const gchar * input)
{
	GtkTreeModel *model;
