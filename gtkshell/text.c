...
setup_entry_cb(struct GSH *gsh, GtkWidget * entry, const gchar * command)
{
	...
	if (gsh && entry && command) {
		...
		GSHCONNECT(gtk_bin_get_child(GTK_BIN(entry)), "activate",
			   cb->entry, cb);
	}
}
...
static void set_options(GSH * gsh, GtkWidget * text_widget)
{
...
static void setup_text_scroller(GSH * gsh, GtkWidget * text)
{
	GtkWidget *scroller;
...
GtkWidget *gsh_text_area(GSH * gsh)
{
	GtkWidget *text;
...
static GtkWidget *setup_unredo_button(GtkWidget * destination,
				      const bool has_toolbar,
				      const gchar * stock, GCallback cb,
				      GtkWidget * text_widget)
{
	GtkWidget *button;

	button = has_toolbar ? GSH_STOCK_ITEM(tool_button, stock)
...
static GtkWidget *setup_toolbar(GSH * gsh)
{
...
static void setup_undo(GSH * gsh)
{
	...
	destination =
	    has_toolbar ? gsh->widgets.app.toolbar : setup_toolbar(gsh);
...
static gchar *get_text(GtkWidget * editor)
{
	GtkTextBuffer *buffer;
...
static void save_to_opened_file(GSH * gsh)
{
	gchar *text;
...
static void get_filename(struct GSH *gsh)
{
	gchar *tmp_filename;
...
static void open_in_editor(GSH * gsh)
{
...
static void recent_file_cb(GtkWidget * widget, gpointer data)
{
	GSH *gsh;
...
static void add_to_recent_files(GSH * gsh, const gchar * filename)
{
	GtkWidget *menu;
