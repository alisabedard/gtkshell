...
static char *
load_cat_title (GKeyFile * kf, CatInfo * inf)
{
  char *ret = NULL;
  ...
  if (!prev)
  ...
  else
    {
      ...
      *menu_item = GTK_WIDGET (prev->data);
      ...
      data = (PtkAppMenuItem *) g_object_get_qdata (G_OBJECT (*menu_item),
...
static void
do_load_dir (int prefix_len, const char *path, GList ** sub_menus)
{
...
static inline void
load_dir (const char *path, GList ** sub_menus)
{
...
static inline bool
ptk_app_menu_item_has_data (GtkMenuItem * item)
{
