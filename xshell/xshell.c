...
static void setup_xshell_gui_layout(XShellGUILayout * layout)
{
	layout->row_count = 0;
...
static void setup_xshell_gui(XShellGUI * gui)
{
	gui->widgets = NULL;
	gui->widget_count = 0;
...
static void setup_xshell(XShell * xsh)
{
	xsh->options = 0;
	setup_xshell_gui(&(xsh->gui));
}

static void delete_widgets(XWidget * head)
{
	XWidget *iter;
...
static void cleanup(XShell * xsh)
{
	Display *dpy;
...
static void show_main_window(XShell * xsh)
{
...
static void setup_main_window(XShell * xsh)
{
	XShellGUI *gui = &(xsh->gui);
	Display *dpy = gui->display;
...
static void apply_toplevel_geometry(XShell * xsh)
{
	XShellGUI *gui = &(xsh->gui);
