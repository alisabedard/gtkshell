...
void draw_widget_text(XWidget * text_widget)
{
	...
	for (i = 0; i < length; i += columns) {
		...
		line_length = (length =
			       strlen(text)) < columns ? length : columns;
		XDrawString(gui->display, text_widget->window, gui->gc, 0,
			    (++line) * font_height, text, line_length);
		text += line_length;
		text = strchr(text, '\n');
	}
}

static void expose(XWidget * text)
{
	draw_widget_text(text);
}

/* Set up for scrolling  */
static void button_press(XWidget * text)
{
	XSHTextData *data;
...
static void button_release(XWidget * text)
{
	expose(text);
}

static void key_press(XWidget * text)
{
	button_press(text);
}

static void configure(XWidget * text)
{
	expose(text);
}

static void setup_events(XWidget * text)
{
		...
		unsigned int rows, const char *command)
{
	...
	xshell_XWidget_new(xsh, parent, x, y,
			   xsh->gui.layout.widget_width =
			   data->font_width * columns,
